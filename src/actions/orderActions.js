...
export const createOrder = (order) => async (dispatch, getState) => {
  ...
  try {
    ...
    localStorage.removeItem("cartTshirtItem");
  } catch (error) {
    dispatch({
...
export const listOrderMine = () => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.get("/api/OrderDetails/mine", {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
...
export const deleteOrder = (orderId) => async (dispatch, getState) => {
  ...
  try {
    const { data } = Axios.delete(`/api/OrderDetails/deleteOrder/${orderId}`, {
      headers: { Authorization: `Bearer ${userInfo.token}` },
    });
  ...
  (checkboxId) => async (dispatch, getState) => {
    ...
    try {
      const { data } = await Axios.put(
        `/api/OrderDetails/assignstatus/${checkboxId.checkboxId}`,
        checkboxId,
        {
          headers: { Authorization: `Bearer ${userInfo.token}` },
        }
      ...
      dispatch({
        type: ORDER_STATUSASSIGN_LIST_SUCCESS,
        payload: data.citymaster,
      });
    } catch (error) {
      const message =
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message;
      dispatch({ type: ORDER_STATUSASSIGN_LIST_FAIL, error: message });
    }
  };
