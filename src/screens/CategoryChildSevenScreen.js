...
function CategoryChildSevenScreen() {
  ...
  const editHandler = (id) => {
    navigate(`/testFrom/` + id, { state: { child: "Child6" } });
  };

  const ViewHandeler = (id) => {
    navigate(`/categorychildeight/` + id);
  };
                  ...
                  for (
                    ...
                    if (
                      categorymasterallList[i].children[j].children[k].children[
                        A
                      ].children[B].children[C].children[D]._id === id
                    ) {
                      NewArry.push({
                        ["parentId"]:
...
export default CategoryChildSevenScreen;
