...
function CategoryChildFourScreen() {
  ...
  const editHandler = (id) => {
    navigate(`/testFrom/` + id, { state: { child: "Child3" } });
  };

  const ViewHandeler = (id) => {
    navigate(`/categorychildfive/` + id);
  };
  ...
  for (let i = 0; i < categorymasterallList?.length; i++) {
    for (let j = 0; j < categorymasterallList[i].children?.length; j++) {
      for (
        ...
        if (categorymasterallList[i].children[j].children[k]._id === parentId) {
          for (
            let A = 0;
            A <
        ...
        for (
          ...
          if (categorymasterallList[i].children[j].children[k]._id === parentId) {
            for (
              ...
              if (
                categorymasterallList[i].children[j].children[k].children[A]._id === id
              ) {
                NewArry.push({
                  ["parentId"]:
...
export default CategoryChildFourScreen;
